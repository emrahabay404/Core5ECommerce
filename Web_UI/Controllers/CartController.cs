...
namespace Web_UI.Controllers
{
   ...
   public class CartController : Controller
   {
         ...
         try
         {
            ...
            if (adresid != 0)
            {
               ...
               foreach (var item in ViewBag.cart)
               {
                  ...
                  nts.NotifiTypeID = 1; nts.SeenStatus = false; C.Notifications.Add(nts); C.SaveChanges();
      ...
      public IActionResult Paydene(int adresid, string walletpay)
      {
         int userid = (int)_userManager.GetUserAsync(User).Result.Id;
         var getwallet = C.Wallets.Where(x => x.Id == userid).FirstOrDefault();
         if (Globals.LoginStatus == 0)
         ...
         else
         {
            ...
            else
            {
               if (walletpay == "true")
               {
                  if (getwallet.Balance < ViewBag.total)
                  {
                     var cart = SessionHelper.GetObjectFromJson<List<Item>>(HttpContext.Session, "cart");
                     ViewBag.cart = cart;
                     ViewBag.total = cart.Sum(item => item.Product.Price * item.Adet);
                     ...
                     foreach (var item in ViewBag.cart)
                     {
                        ...
                        ordetay.CargoID = 10;
                        ordetay.AddressID = adresid;
                        ordetay.OrderProcessID = 6;
                        ordetay.Deliverydate = Convert.ToDateTime(DateTime.Now.ToLongDateString());
                        C.OrderDetails.Add(ordetay);
                        C.SaveChanges();
                        var plsproduct = productRepo.TGet((int)item.Product.ProductId);
                        plsproduct.BuyCount += (int)item.Adet;
      ...
      {
         string abc = Convert.ToString("54576");
         if (abc != string.Empty)
         {
            return View("Okayyy");
         }
